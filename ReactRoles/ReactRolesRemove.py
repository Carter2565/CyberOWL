...
import Events.on_raw_reaction_remove as ReactRolesRemove 
...
@ReactRolesRemove.event()
async def ReactRolesRemove(reaction):
    ...
    if str(channel_id) in ReactRoles[str(server_id)]:
      if str(message_id) in ReactRoles[str(server_id)][str(channel_id)]:
        if (f'{reaction.emoji.name}' in ReactRoles[str(server_id)][str(channel_id)][str(message_id)]):
          # Imported from roles.
          await remove_role_by_id(bot,server_id,user.id,ReactRoles[str(server_id)][str(channel_id)][str(message_id)][str(reaction.emoji.name)])
        elif(f'{emoji}' in ReactRoles[str(server_id)][str(channel_id)][str(message_id)]):
          # Imported from roles.
          await remove_role_by_id(bot,server_id,user.id,ReactRoles[str(server_id)][str(channel_id)][str(message_id)][emoji])

  # print(f'{user} removed reaction {emoji} to message {message}')  # Debug